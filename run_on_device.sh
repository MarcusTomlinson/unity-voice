...
build() {
    exec_with_ssh "[ $CODE_DIR/debian/control -nt indicator-secret-agent-build-deps*deb ] && $SUDO mk-build-deps --tool=\\\"apt-get -y --no-install-recommends\\\" --build-dep --install $CODE_DIR/debian/control"
